...
const getAffiliateMenuIcon = (resourceKey: string) => {
  const normalized = resourceKey.trim().toUpperCase();

  if (
    normalized === "AFFILIATE_DASHBOARD" ||
  ...
  if (normalized === "AFFILIATE_ONBOARDING" || normalized.includes("ONBOARDING")) {
    return <i className="fa-solid fa-graduation-cap h-5 w-5" aria-hidden="true"></i>;
  }

  if (
    normalized === "AFFILIATE_ADMIN" ||
    normalized === "ADMIN" ||
  ...
  if (normalized.includes("AFFILIATE")) {
    return <i className="fa-solid fa-shop h-5 w-5" aria-hidden="true"></i>;
  }
...
const AffiliateSidebar = ({
  ...
  useEffect(() => {
    ...
    loadPortalSidebarData({
      portalKey: "AFFILIATE",
      resolveIcon: getAffiliateMenuIcon,
    })
  ...
  const isProfileActive =
    location.pathname === "/affiliate/profile" ||
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-white/10 p-4">
        {isOpen ? (
          <Link to="/affiliate" className="mx-auto transition-transform hover:scale-[1.03]">
            <img
        ...
        <button
          ...
          aria-label={isOpen ? "Tutup sidebar affiliate" : "Buka sidebar affiliate"}
        >
          <svg
            ...
            <div key={item.id} className="group relative overflow-hidden">
              <Link
                ...
                className={`flex w-full items-center rounded-lg p-3 transition-colors ${
                  active
                    ? "rounded-l-lg rounded-r-none border border-r-0 border-white/35 bg-gradient-to-r from-[#0f766e] via-gray-950 to-gray-950 font-semibold text-white"
              ...
              {active && isOpen ? (
                <div
                  className="absolute right-0 top-1/2 h-0 w-0 -translate-y-1/2 border-b-[24px] border-r-[24px] border-t-[24px] border-b-gray-950 border-r-[#f0fdfa] border-t-gray-950"
                  aria-hidden="true"
                ></div>
              ) : null}
          ...
          <div className={`flex gap-3 px-2 ${isOpen ? "items-start" : "flex-col items-center"}`}>
            <button
              ...
              className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-sm font-semibold text-white shadow-md transition ${
                isProfileActive
                  ? "bg-[#0f766e] ring-2 ring-white/25"
                  : "bg-[#14b8a6] hover:bg-[#0f766e]"
              }`}
              aria-label="Buka profil affiliate"
            >
            ...
            {isOpen ? (
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 truncate text-xs uppercase tracking-[0.22em] text-gray-400">
                  {user.roleName || "Affiliate Workspace"}
                </p>

                <button
                  type="button"
                  onClick={onLogout}
                  className="mt-2 flex items-center gap-2 text-sm text-gray-300 transition-colors hover:text-teal-300"
                >
