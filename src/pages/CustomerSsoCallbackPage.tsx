...
const CustomerSsoCallbackPage = () => {
  ...
  useEffect(() => {
    ...
    const login = async () => {
      ...
      if (!normalizedToken) {
        setErrorMessage("Token customer tidak ditemukan.");
        return;
      }

      try {
        const res = await apiFetch("/customer-sso/login", {
          method: "POST",
          credentials: "include",
        ...
        if (!res.ok) {
          setErrorMessage(
            getApiErrorMessage(data, "Token customer tidak valid atau sudah kedaluwarsa.")
          );
          return;
        }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#edf4fb] px-4 text-slate-900">
      <section className="w-full max-w-md rounded-lg border border-[#dbe5f1] bg-white px-6 py-6 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.45)]">
        <h1 className="text-lg font-semibold text-slate-950">Masuk ke OMS Customer</h1>
        {errorMessage ? (
          <>
...
export default CustomerSsoCallbackPage;
