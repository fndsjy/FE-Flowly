...
const getCommunityMenuIcon = (resourceKey: string) => {
  const normalized = resourceKey.trim().toUpperCase();

  if (
    normalized === "COMMUNITY_DASHBOARD" ||
  ...
  if (normalized === "COMMUNITY_ONBOARDING" || normalized.includes("ONBOARDING")) {
  ...
  if (normalized.includes("COMMUNITY")) {
    return <i className="fa-solid fa-people-group h-5 w-5" aria-hidden="true"></i>;
  }
...
const CommunitySidebar = ({
  ...
  useEffect(() => {
    ...
    loadPortalSidebarData({
      portalKey: "COMMUNITY",
      resolveIcon: getCommunityMenuIcon,
    })
    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-white/10 p-4">
        ...
        <button
          ...
          aria-label={isOpen ? "Tutup sidebar community" : "Buka sidebar community"}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5"
        ...
        {user ? (
          <div className={`flex gap-3 px-2 ${isOpen ? "items-start" : "flex-col items-center"}`}>
            <button
              ...
              aria-label="Buka profil community"
            >
