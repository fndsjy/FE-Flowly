...
type Stage = { onboardingStageTemplateId: string; programType: string; stageNumber: number; stageLabel: string; stageTitle: string };
...
const PROGRAM_OPTIONS = [{ value: "ONBOARDING", label: "Onboarding" }, { value: "LEARNING", label: "LMS" }] as const;
const getProgramLabel = (programType: string) => PROGRAM_OPTIONS.find((item) => item.value === programType)?.label ?? programType;
...
const fileUrl = (
  ...
  if (fileType !== null && !Number.isNaN(fileType)) {
    query.set("fileType", String(fileType));
  }
...
export default function OnboardingAdministratorMaterialsPage() {
  ...
  const sortedAssignments = useMemo(() => [...assignments].sort((a, b) => a.portalOrderIndex - b.portalOrderIndex || a.programType.localeCompare(b.programType) || a.stageNumber - b.stageNumber || a.orderIndex - b.orderIndex || a.materialTitle.localeCompare(b.materialTitle)), [assignments]);
  const summary = useMemo(() => ({
    source: materials.length,
    placed: sortedAssignments.length,
    portals: new Set(sortedAssignments.map((item) => item.portalKey)).size,
    slots: new Set(sortedAssignments.map((item) => `${item.portalKey}-${item.programType}-${item.stageNumber}`)).size,
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return sortedAssignments.filter((item) => {
      ...
      if (programFilter !== "ALL" && item.programType !== programFilter) return false;
      if (!term) return true;
      return [item.programType, getProgramLabel(item.programType), item.materialCode, item.materialTitle, item.materialDescription, item.portalLabel, item.stageLabel, item.assignmentNote, item.materialTypes.join(" "), item.files.map((file) => file.title ?? file.fileName).join(" ")].some((value) => includes(value, term));
    });
  }, [portalFilter, programFilter, search, sortedAssignments]);
  const displayedPortals = useMemo(() => {
    const base = portals
      .filter((portal) => portalFilter === "ALL" || portal.portalKey === portalFilter)
      .map((portal) => ({
        ...portal,
        stages: portal.stages.filter((stage) => programFilter === "ALL" || stage.programType === programFilter),
      }))
  ...
  }, [filteredAssignments, portalFilter, portals, programFilter, search]);
  const stageAssignmentsByMaterialId = useMemo(() => {
    if (!pickerTarget) return new Map<number, Assignment>();
    return new Map(
      sortedAssignments
        .filter((item) => item.onboardingStageTemplateId === pickerTarget.onboardingStageTemplateId)
  ...
  const openPicker = (portal: Portal, stage: Stage) => {
    setPickerTarget({ onboardingStageTemplateId: stage.onboardingStageTemplateId, programType: stage.programType, portalKey: portal.portalKey, portalLabel: portal.portalLabel, stageNumber: stage.stageNumber, stageLabel: stage.stageLabel, stageTitle: stage.stageTitle });
    setPickerSearch("");
    setPickerType("ALL");
  };
      ...
      <div className="space-y-6">
        ...
        <section className={`${panel} p-5 md:p-6`}>
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_14rem_14rem]">
            ...
            <select value={programFilter} onChange={(event) => setProgramFilter(event.target.value)} className="rounded-[18px] border border-[#d8e0e8] bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-slate-400">
            ...
            <section key={portal.onboardingPortalTemplateId} className={`${panel} p-5 md:p-6`}>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                {portal.stages.map((stage) => {
                  const stageRows = portalRows.filter((item) => item.onboardingStageTemplateId === stage.onboardingStageTemplateId);
                  return (
                    <section key={stage.onboardingStageTemplateId} className="rounded-[24px] border border-[#ebeff4] bg-[linear-gradient(180deg,#fbfdff_0%,#f8fafc_100%)] p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Tahap {getProgramLabel(stage.programType)}</p>
                          ...
                          <article key={item.assignmentId} className="rounded-[18px] border border-[#ebeff4] bg-white p-4">
                            ...
                            <div className="mt-3 space-y-2">
                              {item.files.slice(0, 2).map((file) => (
                                <div key={file.id} className="flex items-center justify-between gap-3 rounded-[14px] border border-[#ebeff4] bg-[#f8fafc] px-3 py-2 text-sm text-slate-700">
                                  <a href={fileUrl(file.fileName, file.url, file.fileType)} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate underline-offset-2 hover:underline">{file.title ?? file.fileName}</a>
                                  <div className="flex shrink-0 items-center gap-2">
                                    <a href={fileUrl(file.fileName, file.url, file.fileType)} target="_blank" rel="noreferrer" className="text-[11px] font-semibold text-slate-500 underline-offset-2 hover:text-slate-800 hover:underline">Preview</a>
                                    <a href={fileUrl(file.fileName, file.url, file.fileType, "attachment")} className="text-[11px] font-semibold text-blue-600 underline-offset-2 hover:text-blue-800 hover:underline">Download</a>
                                  </div>
                                </div>
                              ))}
                            </div>
