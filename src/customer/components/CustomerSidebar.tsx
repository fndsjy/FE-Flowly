...
const getCustomerMenuIcon = (resourceKey: string) => {
  const normalized = resourceKey.trim().toUpperCase();

  if (
    normalized === "CUSTOMER_DASHBOARD" ||
  ...
  if (normalized === "CUSTOMER_ONBOARDING" || normalized.includes("ONBOARDING")) {
  ...
  if (normalized === "CUSTOMERS" || normalized.includes("CUSTOMER")) {
    return <i className="fa-solid fa-address-book h-5 w-5" aria-hidden="true"></i>;
  }
...
const CustomerSidebar = ({
  ...
  useEffect(() => {
    ...
    loadPortalSidebarData({
      portalKey: "CUSTOMER",
      resolveIcon: getCustomerMenuIcon,
    })
  ...
  const isItemActive = (item: CustomerSidebarItem) => {
    ...
    >
      <div className="flex items-center justify-between border-b border-white/10 p-4">
        ...
        <button
          ...
          aria-label={isOpen ? "Tutup sidebar customer" : "Buka sidebar customer"}
        >
          <svg
        ...
        {user ? (
          <div className={`flex gap-3 px-2 ${isOpen ? "items-start" : "flex-col items-center"}`}>
            <button
              ...
              aria-label="Buka profil customer"
            >
